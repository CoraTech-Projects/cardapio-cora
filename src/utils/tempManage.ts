...
entries.forEach(([key, data]: [string, any]) => {
    cron.schedule(convertToCron(data.entrada), () => {
        console.log(`\x1b[32m [ TAREFA ]: \x1b[0m Início do período ${data.periodo} - ${hoje}`);
    });

    cron.schedule(convertToCron(data.saida), () => {
        console.log(`\x1b[31m [ TAREFA ]: \x1b[0m Fim do período ${data.periodo} - ${hoje}`);
    });

    cron.schedule(convertToCron(data.horario_lanche), () => {
        console.log(`\x1b[33m [ TAREFA ]: \x1b[0m Lanche do período ${data.periodo} - ${hoje}`);
    });

    if(data.horario_especial && data.horario_especial.horario) {
        cron.schedule(convertToCron(data.horario_especial.horario), () => {
            console.log(`\x1b[35m [ TAREFA ]: \x1b[0m Evento especial do período ${data.periodo} - ${hoje}: ${data.horario_especial.descricao}`);
        });
    }
});