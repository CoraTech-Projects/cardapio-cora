...
const TTS_MODEL_PATH =
  path.resolve(process.cwd(), 'src', 'IA', 'models') + '/model.onnx';
const MODEL_PATH =
  path.resolve(process.cwd(), 'src', 'IA', 'models') + '/model-llm-4g.gguf';
...
const CONFIG_PATH = './src/IA/models/model.onnx.json';
...
function initModels(): void {
  ...
  ort.InferenceSession.create(TTS_MODEL_PATH)
    .then((s) => {
      model = s;
  ...
  console.log(
    '\x1b[32m [ MODEL_LLAMA ]: \x1b[0m Modelo LLM sendo carregado...'
  );
...
function toWavHeader(audio: Float32Array): Buffer {
...
async function generateTTS(text: string): Promise<Buffer | any> {
  ...
  console.log(
    '\x1b[34m [ TTS ]: \x1b[0m Texto convertido para fonemas IPA:',
    rawIpa
  );
  ...
  const feeds = {
    ...
    scales: new ort.Tensor(
      'float32',
      Float32Array.from([0.667, 1.3, 0.8]),
      [3]
    ),
  };
