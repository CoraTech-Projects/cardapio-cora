...
function isConsonant(
    ...
    return /[bcdfghjklmnpqrstvwxyzç]/i.test(
        char
    );
}
    ...
    for (
        ...
        if (
            ...
            if (
                ...
                if (
                    /^(br|cr|dr|fr|gr|pr|tr|vr|bl|cl|fl|gl|pl|qu|gu|ch|lh|nh)$/.test(
                        cluster
                    )
                ) {
                    continue;
...
function detectStress(
    ...
    if (
        /(a|e|o|em|ens|am|ão|ões|ãe|õe|ã)$/i.test(
            cleaned
        )
    ) {
...
function applyPhonology(
    ...
    return rules.reduce(
        (value, rule) =>
            value.replace(rule.pattern, rule.replace),
        text
    );
}
